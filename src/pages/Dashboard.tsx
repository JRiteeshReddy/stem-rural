...
export default function Dashboard() {
  ...
  if (isStudent) {
    // Student view data preparation
    const subjects = [
      { name: "Mathematics", icon: "📐", level: 1, pct: 0, color: "bg-yellow-500" },
      { name: "Chemistry", icon: "🧪", level: 1, pct: 0, color: "bg-orange-500" },
      { name: "Biology", icon: "🌱", level: 1, pct: 0, color: "bg-green-500" },
      { name: "Computer Science", icon: "💻", level: 1, pct: 0, color: "bg-blue-500" },
      { name: "Robotics", icon: "🤖", level: 1, pct: 0, color: "bg-purple-500" },
      { name: "Astronomy", icon: "🌟", level: 1, pct: 0, color: "bg-indigo-500" },
    ];

    const goals = [
      { text: "Complete any 1 Test", achieved: (user.totalTestsCompleted || 0) >= 1 },
      { text: "Earn 10 XP", achieved: (user.credits || 0) >= 10 },
      {
        text: "Access any course today",
        achieved: (optStudentCourses || []).some(
          (c: any) => c.lastAccessed && Date.now() - c.lastAccessed < 24 * 60 * 60 * 1000
        ),
      },
    ...
    const achievements = [
      { name: "Math Wizard", icon: "🧙", earned: (user.credits || 0) >= 50 },
      { name: "Coding Hero", icon: "💻", earned: (user.credits || 0) >= 100 },
      { name: "Science Explorer", icon: "🔬", earned: (user.totalTestsCompleted || 0) >= 5 },
      { name: "Problem Solver", icon: "🧩", earned: (user.credits || 0) >= 150 },
      { name: "Robot Master", icon: "🤖", earned: (user.totalTestsCompleted || 0) >= 10 },
    ];
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="min-h-screen bg-transparent">
            ...
            {/* Control Center Tabs - keep existing PixelCard with Tabs */}
            <PixelCard variant="orange" className="p-6">
              <Tabs value={activeTab} onValueChange={setActiveTab}>
                {/* ... keep all existing TabsList, TabsTrigger, and TabsContent elements */}
              </Tabs>
            </PixelCard>
          </div>
